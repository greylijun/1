...
class HistoryQueryHour{
    ...
    function QueryHour($HourStart,$HourEnd,$changeValue){
        ...
        $sql = $conn->query("select sum((case `energy_big_type` when '水' then `".$changeValue."` else 0 end)) 'water',sum((case `energy_big_type` when '电' then `".$changeValue."` else 0 end)) 'electrical',sum((case `energy_big_type` when '煤' then `".$changeValue."` else 0 end)) 'coal',sum((case `energy_big_type` when '油' then `".$changeValue."` else 0 end)) 'oil',sum((case `energy_big_type` when '天然气' then `".$changeValue."` else 0 end)) 'gas',sum((case `energy_big_type` when '蒸汽' then `".$changeValue."` else 0 end)) 'steam',sum((case `energy_big_type` when '冷量' then `".$changeValue."` else 0 end)) 'cold' from `t_back_step` where `enable`='启用' and `enter_tsn`='".$enter_tsn."' and DATE_FORMAT(import_time,'%Y/%m/%d %H:%m') between '".$HourStart."' and '".$HourEnd."'");
        ...
        /*}else if($typeValue=="industry"){
            $sql = $conn->query("select b.`industry_type`,sum(a.`".$changeValue."`) as sum_value from `t_daily_data` a left join `t_enterprise_info` b on a.enter_tsn=b.tsn where a.`enable`='启用' and DATE_FORMAT(import_time,'%Y/%m/%d %H:%m') between '".$HourStart."' and '".$HourEnd."' group by b.`industry_type`");
            ...
            $sql = $conn->query("select b.`region`,sum(a.`".$changeValue."`) as sum_value from `t_daily_data` a left join `t_enterprise_info` b on a.enter_tsn=b.tsn where a.`enable`='启用' and DATE_FORMAT(import_time,'%Y/%m/%d %H:%m') between '".$HourStart."' and '".$HourEnd."'  group by b.`region`");
            while($result = mysqli_fetch_array($sql)){
...
$obj = new HistoryQueryHour();
$obj->QueryHour($_GET["hourStart"],$_GET["hourEnd"],$_GET["changeValue"]);
