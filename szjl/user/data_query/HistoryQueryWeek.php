...
//            $sql = $conn->query("select b.`industry_type`,sum(a.`".$changeValue."`) as sum_value from `t_daily_data` a left join `t_enterprise_info` b on a.enter_tsn=b.tsn where a.`enable`='启用' and DATE_FORMAT(import_time,'%Y') = '".$yearValue."' DATE_FORMAT(import_time,'%u') >= '".$WeekStart."' and DATE_FORMAT(import_time,'%u') <= '".$WeekEnd."' group by b.`industry_type`");
...
//            echo json_encode($data);
//        }else{
//            $sql = $conn->query("select b.`region`,sum(a.`".$changeValue."`) as sum_value from `t_daily_data` a left join `t_enterprise_info` b on a.enter_tsn=b.tsn where a.`enable`='启用' and DATE_FORMAT(import_time,'%Y') = '".$yearValue."' DATE_FORMAT(import_time,'%u') >= '".$WeekStart."' and DATE_FORMAT(import_time,'%u') <= '".$WeekEnd."' group by b.`region`");
//            while($result = mysqli_fetch_array($sql)){
...
$obj = new HistoryQueryWeek();
$obj->QueryWeek($_GET["yearValue"],$_GET["weekStart"],$_GET["weekEnd"],$_GET["changeValue"]);
