...
//            $sql = $conn->query("select b.`industry_type`,sum(a.`".$changeValue."`) as sum_value from `t_daily_data` a left join `t_enterprise_info` b on a.enter_tsn=b.tsn where a.`enable`='启用' and DATE_FORMAT(a.import_time,'%Y') = '".$yearValue."' group by b.`industry_type`");
...
//            echo json_encode($data);
//        }else{
//            $sql = $conn->query("select b.`region`,sum(a.`".$changeValue."`) as sum_value from `t_daily_data` a left join `t_enterprise_info` b on a.enter_tsn=b.tsn where a.`enable`='启用' and DATE_FORMAT(a.import_time,'%Y') = '".$yearValue."' group by b.`region`");
//            while($result = mysqli_fetch_array($sql)){
...
$obj = new HistoryQueryYear();
$obj->QueryYear($_GET["yearValue"],$_GET["changeValue"]);
