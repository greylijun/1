...
$fixed = $_GET['if_fixed'];
$sql = "select `instrument_name`,`model_num`,`path_number`,`failure_time`,`recovery_time`,`if_fixed`,`enterprise_name` from (`t_collecting_point` as `a` inner join `t_enterprise_info` as `b` on `a`.`enter_tsn`=`b`.`tsn`) inner join `t_equipment_log` as `c` on `a`.`tsn`=`c`.`point_tsn` where `b`.`review_status`='通过审核'";
