...
$sql = "select `enterprise_name`,`license_registration_number`,`organization_code`,`legal_person`,`business_contacts`,`contact_number`,`region`,`industry_type`,`second_industry_type`,`business_phone`,`zip_code`,`E_mail`,`fax`,`registered_address`,`production_address`,`unit_image_path`,`about_enter` from `t_enterprise_info` where `review_status`='通过审核' and `organization_code`='$code'";
$result = mysqli_query($conn,$sql);
if($result){
    $row = mysqli_fetch_array($result);
}
$sqlForPoint = "select count(`path_number`) as `path_num` from `t_collecting_point` as `a` left join `t_enterprise_info` as `b` on `a`.`enter_tsn`=`b`.`tsn` where `b`.`review_status`='通过审核' and `b`.`organization_code`='$code'";
$resultForPoint = mysqli_query($conn,$sqlForPoint);
if($resultForPoint){
    $rowForPoint = mysqli_fetch_array($resultForPoint);
}
$sqlForLastEnergy = "select sum(`daily_equival_value`) as `last_sum` from `t_daily_data` as `a` left join `t_enterprise_info` as `b` on `a`.`enter_tsn`=`b`.`tsn` where `b`.`review_status`='通过审核' and `b`.`organization_code`='$code' and date(`import_time`)=CURRENT_DATE -1 and `enable`='启用'";
...
$enter_data = array(
    ...
    'industry_type'=>$row['industry_type'],                //行业类型
    'second_industry_type'=>$row['second_industry_type'],  //二级行业
